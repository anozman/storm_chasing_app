...
def sample_pipeline():
    ...
    # Step 1: Find the latest scan
    latest_files = find_latest_scan(radar_id)
    if not latest_files:
        print("No files found for the latest scan.")
        return

    # Step 2: Download the chunks
    local_file_paths = download_chunks(latest_files, download_dir=output_dir)

    # Step 3: Assemble the chunks into a single file
